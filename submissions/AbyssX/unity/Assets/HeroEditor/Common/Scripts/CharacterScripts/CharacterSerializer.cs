	...
	public partial class Character
	{
		...
		private IEnumerable<ItemSprite> GetWeaponCollection(WeaponType weaponType)
		{
			switch (weaponType)
			{
				...
				case WeaponType.FirearmsPaired: return SpriteCollection.Firearm1H;
		...
		private static void RestoreFromString(ref Sprite sprite, SpriteRenderer renderer, IEnumerable<ItemSprite> collection, string serialized, bool byRendererName = false)
		{
		...
		private static void RestoreFromString(ref List<Sprite> sprites, List<SpriteRenderer> renderers, IEnumerable<ItemSprite> collection, string serialized)
		{
			if (string.IsNullOrEmpty(serialized))
			{
