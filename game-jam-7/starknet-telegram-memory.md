# StarkMem — On-chain Memory (Telegram Mini-App)
...
Play directly inside Telegram via the StarkMem Mini App. Players tap to flip cards, try to find matching pairs, and finish levels with the fewest moves and fastest time to earn higher scores. Game state and matching logic are persisted on-chain (Starknet), while the Dojo Telegram SDK handles account abstraction and gasless session key signing.
