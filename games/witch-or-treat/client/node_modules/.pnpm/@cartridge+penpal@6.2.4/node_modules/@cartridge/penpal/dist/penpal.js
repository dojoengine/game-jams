var Penpal = (function () {
  ...
  var connectCallReceiver = ((info, serializedMethods, log) => {
    ...
    const handleMessageEvent = event => {
      ...
      new Promise(resolve => resolve(serializedMethods[methodName].call(serializedMethods, event.origin).apply(serializedMethods, args))).then(createPromiseHandler(Resolution.Fulfilled), createPromiseHandler(Resolution.Rejected));
    };
      ...
      const handleMessage = event => {
        ...
        if (event.data.penpal === MessageType.SynAck) {
          ...
          if (callSender) {
            window.removeEventListener(NativeEventType.Message, handleMessage);
            stopConnectionTimeout();
            resolve(Object.assign(Object.assign({}, callSender), {
              origin: event.origin
            }));
          }
        }
