...
import { AccountController, ChainController, ConnectionController, ConnectorController, CoreHelperUtil, ModalController, SnackController } from '@reown/appkit-controllers';
...
describe('W3mProfileView - Render', () => {
    ...
    test('should render profile information correctly', async () => {
        const element = await fixture(html `<w3m-profile-view></w3m-profile-view>`);
...
describe('W3mProfileView - Functions', () => {
    ...
    test('should handle account switching', async () => {
        ...
        getAuthConnectorMock.mockReturnValue(MOCK_AUTH_CONNECTOR);
        const element = await fixture(html `<w3m-profile-view></w3m-profile-view>`);
        await element.updateComplete;
        const switchAccount = {
            address: '0xabc...def',
        ...
        await element.onSwitchAccount(switchAccount);
        await new Promise(resolve => setTimeout(resolve, 0));
        expect(setPreferredAccountTypeMock).toHaveBeenCalledWith('eoa', switchAccount.namespace);
        expect(setShouldUpdateToAddressMock).toHaveBeenCalledWith(switchAccount.address, switchAccount.namespace);
    });
    ...
    test('should throw error when no address is provided', async () => {
        ...
        await expect(fixture(html `<w3m-profile-view></w3m-profile-view>`)).rejects.toThrow('w3m-profile-view: No account provided');
    });
});
//# sourceMappingURL=w3m-profile-view.test.js.map