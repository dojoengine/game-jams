...
describe('W3mSwapSelectTokenView', () => {
    ...
    it('should render initial state with token lists', async () => {
        const element = await fixture(html `<w3m-swap-select-token-view></w3m-swap-select-token-view>`);
        await element.updateComplete;
        const searchInput = element.shadowRoot?.querySelector('[data-testid="swap-select-token-search-input"]');
    ...
    it('should select source token and go back', async () => {
        ...
        const element = await fixture(html `<w3m-swap-select-token-view></w3m-swap-select-token-view>`);
        await element.updateComplete;
        const tokenItem = element.shadowRoot?.querySelector('[data-testid="swap-select-token-item-DAI"]');
        tokenItem?.click();
    ...
    it('should handle scroll events', async () => {
        const element = await fixture(html `<w3m-swap-select-token-view></w3m-swap-select-token-view>`);
        await element.updateComplete;
        const suggestedTokensContainer = element.shadowRoot?.querySelector('.suggested-tokens-container');
        const tokensList = element.shadowRoot?.querySelector('.tokens');
        suggestedTokensContainer?.dispatchEvent(new Event('scroll'));
        tokensList?.dispatchEvent(new Event('scroll'));
        expect(suggestedTokensContainer?.style.getPropertyValue('--suggested-tokens-scroll--left-opacity')).to.exist;
...
//# sourceMappingURL=w3m-swap-select-token-view.test.js.map