...
import { ApiController, ConnectorController, CoreHelperUtil, RouterController } from '@reown/appkit-controllers';
...
describe('W3mConnectAnnouncedWidget', () => {
    ...
    it('should not render anything if there are no announced connectors', async () => {
        ...
        const element = await fixture(html `<w3m-connect-announced-widget></w3m-connect-announced-widget>`);
    ...
    it('should route to AllWallets on mobile for WalletConnect', async () => {
        ...
        const element = await fixture(html `<w3m-connect-announced-widget></w3m-connect-announced-widget>`);
        const walletSelector = HelpersUtil.getByTestId(element, `wallet-selector-${WALLET_CONNECT_CONNECTOR.id}`);
...
//# sourceMappingURL=w3m-connect-announced-widget.test.js.map