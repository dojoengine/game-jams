...
describe('W3mSwapView', () => {
    ...
    it('should render initial state with token details', async () => {
        const element = await fixture(html `<w3m-swap-view></w3m-swap-view>`);
    ...
    it('should handle token switching', async () => {
        ...
        const switchButton = element.shadowRoot?.querySelector('.replace-tokens-button-container button');
        switchButton?.click();
    ...
    it('should handle caipAddress change', async () => {
        ...
        const caipAddressCallArgs = subscribeKeySpy.mock.calls
            ...
            .map(call => call[1]);
        const callbacks = caipAddressCallArgs.map((call) => call);
    ...
    it('should set initial state with preselected tokens', async () => {
        const element = await fixture(html `<w3m-swap-view
        initialParams='{"fromToken": "AAAA","toToken":"BBBB","amount":"321.123"}'
      ></w3m-swap-view>`);
        await element.updateComplete;
        vitestExpect(SwapController.setSourceToken).toHaveBeenCalledWith(vitestExpect.objectContaining({
            symbol: 'AAAA'
        }));
        vitestExpect(SwapController.setToToken).toHaveBeenCalledWith(vitestExpect.objectContaining({
            symbol: 'BBBB'
        }));
