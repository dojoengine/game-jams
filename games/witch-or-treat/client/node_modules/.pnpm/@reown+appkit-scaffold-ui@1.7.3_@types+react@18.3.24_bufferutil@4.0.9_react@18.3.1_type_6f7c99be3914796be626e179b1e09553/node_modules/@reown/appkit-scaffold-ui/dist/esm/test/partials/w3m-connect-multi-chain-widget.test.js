...
const MOCK_MULTI_CHAIN_CONNECTOR = {
    id: 'mockMultiChain',
...
describe('W3mConnectMultiChainWidget', () => {
    ...
    it('should not render anything if there are no multi-chain connectors', async () => {
        ...
        const element = await fixture(html `<w3m-connect-multi-chain-widget></w3m-connect-multi-chain-widget>`);
    ...
    it('should render multi-chain connectors except WalletConnect', async () => {
        ...
        const mockWalletSelector = HelpersUtil.getByTestId(element, `wallet-selector-${MOCK_MULTI_CHAIN_CONNECTOR.id}`);
        const walletConnectSelector = HelpersUtil.getByTestId(element, `wallet-selector-${WALLET_CONNECT_CONNECTOR.id}`);
    ...
    it('should set active connector and route to ConnectingMultiChain on click', async () => {
        ...
        const element = await fixture(html `<w3m-connect-multi-chain-widget></w3m-connect-multi-chain-widget>`);
        const walletSelector = HelpersUtil.getByTestId(element, `wallet-selector-${MOCK_MULTI_CHAIN_CONNECTOR.id}`);
    ...
    it('should handle tabIdx property', async () => {
        ...
        const element = await fixture(html `<w3m-connect-multi-chain-widget .tabIdx=${2}></w3m-connect-multi-chain-widget>`);
        const walletSelector = HelpersUtil.getByTestId(element, `wallet-selector-${MOCK_MULTI_CHAIN_CONNECTOR.id}`);
        expect(walletSelector.getAttribute('tabIdx')).toBe('2');
    });
});
//# sourceMappingURL=w3m-connect-multi-chain-widget.test.js.map