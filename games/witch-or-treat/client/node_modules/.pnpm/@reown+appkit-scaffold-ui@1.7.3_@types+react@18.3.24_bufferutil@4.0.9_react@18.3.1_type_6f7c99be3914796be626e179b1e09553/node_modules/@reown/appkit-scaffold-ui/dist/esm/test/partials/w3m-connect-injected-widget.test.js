...
import { ApiController, ConnectionController, ConnectorController, CoreHelperUtil, RouterController } from '@reown/appkit-controllers';
...
describe('W3mConnectInjectedWidget', () => {
    ...
    it('should not render anything if there are no injected connectors', async () => {
        const element = await fixture(html `<w3m-connect-injected-widget .connectors=${[]}></w3m-connect-injected-widget>`);
        expect(element.style.display).toBe('none');
    });
    it('should render injected connectors', async () => {
        const element = await fixture(html `<w3m-connect-injected-widget
