...
describe('W3mSwapPreviewView', () => {
    beforeEach(() => {
        ...
        vi.spyOn(SwapController, 'getTransaction').mockImplementation(async () => mockSwapState.swapTransaction);
    ...
    it('should render initial state with token details', async () => {
        const element = await fixture(html `<w3m-swap-preview-view></w3m-swap-preview-view>`);
    ...
    it('should handle swap transaction', async () => {
        ...
        await actionButton?.click();
        await element.updateComplete;
        expect(SwapController.sendTransactionForSwap).toHaveBeenCalledWith(mockSwapState.swapTransaction);
    });
...
//# sourceMappingURL=w3m-swap-preview-view.test.js.map