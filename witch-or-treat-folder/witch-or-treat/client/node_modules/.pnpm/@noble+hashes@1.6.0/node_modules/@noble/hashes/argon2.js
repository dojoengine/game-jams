...
function Hp(A, dkLen) {
    ...
    for (; dkLen - pos > 64; pos += 32) {
        const Vh = blake2b_js_1.blake2b.create({}).update(V);
        Vh.digestInto(V);
        Vh.destroy();
...
function argon2Init(type, password, salt, opts) {
    ...
    // Validation
    (0, _assert_js_1.anumber)(p);
    (0, _assert_js_1.anumber)(dkLen);
    (0, _assert_js_1.anumber)(m);
    (0, _assert_js_1.anumber)(t);
    (0, _assert_js_1.anumber)(version);
    ...
    */
    ...
    for (const [k, i] of Object.entries({ p, dkLen, m, t, version, type })) {
        if (!isUint32(i))
            throw new Error('Argon2: invalid parameter=' + k + ', expected uint32');
...
function argon2Output(B, p, laneLen, dkLen) {
    ...
    const res = (0, utils_js_1.u8)(Hp(B_final, dkLen));
    B_final.fill(0);
    return res;
}
...
async function argon2Async(type, password, salt, opts) {
    const { mP, p, t, version, B, laneLen, lanes, segmentLen, dkLen, perBlock, asyncTick } = argon2Init(type, password, salt, opts);
    // Pre-loop setup
    // [address, input, zero_block] format so we can pass single U32 to block function
    const address = new Uint32Array(3 * 256);
    address[256 + 6] = mP;
...
const argon2dAsync = (password, salt, opts) => argon2Async(0 /* Types.Argond2d */, password, salt, opts);
exports.argon2dAsync = argon2dAsync;
const argon2iAsync = (password, salt, opts) => argon2Async(1 /* Types.Argon2i */, password, salt, opts);
exports.argon2iAsync = argon2iAsync;
const argon2idAsync = (password, salt, opts) => argon2Async(2 /* Types.Argon2id */, password, salt, opts);
